...
bool compareCandidatePaths(const CandidatePath* path1,
    const CandidatePath* path2) {
  return (path1->distance + path1->estimate) >
    (path2->distance + path2->estimate);
}

vector<string> AStar::GetPath(const IGraph* graph, const std::string& from,
  ...
  while (!possible_paths.empty()) {
    ...
    if (visited.find(path_end) == visited.end()) {
      ...
      const vector<IGraphNode*>& next_steps
        = path_end_node->GetNeighbors();
      ...
      for (IGraphNode* next : next_steps) {
        ...
        possible_paths.push(
            new CandidatePath(
              ...
              candidate->distance +
              cost->Calculate(path_end_node->GetPosition(),
                next->GetPosition()),
              heuristic->Calculate(next->GetPosition(),
                terminal_node->GetPosition())));
      }
    }
  }
}

std::vector<std::string> DepthFirstSearch::GetPath(const IGraph* graph,
    const std::string& from,
    const std::string& to)
  const {
    unordered_set<string> visited;  // don't check nodes we've already visited
    queue<CandidatePath*> possible_paths;  // queue of all paths we're
    // considering in BFS
