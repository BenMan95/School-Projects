...
unordered_map<V, int>* count_value_occurrences
(const unordered_map<T, V>* to_count) {
...
template <class T, class V>
unordered_map<T, bool>* where_equal(const unordered_map<T, int>* mapping,
    V equal_to) {
...
class GraphUtils {
 public:
  static unordered_map<string, int>*
    ConnectedComponents(const IGraph* graph);
  static OSMGraph* Filter(const IGraph* original,
      const unordered_map<string, bool>* filter);
  static OSMGraph* FilterToLargestConnectedComponent(const IGraph* graph);
 private:
  static void dfs_visit(const IGraphNode* n,
      unordered_map<string, int>* record, int index);
};

unordered_map<string, int>* GraphUtils::ConnectedComponents
(const IGraph* graph) {
...
OSMGraph* GraphUtils::Filter(const IGraph* original,
    const unordered_map<string, bool>* filter) {
  // NOTE: does not preserve the type of the graph that was input, always
  // gives back an OSMGraph
...
OSMGraph* GraphUtils::FilterToLargestConnectedComponent
(const IGraph* original) {
  unordered_map<string, int>* cc_mapping
    = GraphUtils::ConnectedComponents(original);
  unordered_map<int, int>* cc_sizes = count_value_occurrences(cc_mapping);
  int largest_subgraph_idx = argmax(cc_sizes);
  unordered_map<string, bool>* in_largest_cc
    = where_equal(cc_mapping, largest_subgraph_idx);
...
void GraphUtils::dfs_visit(const IGraphNode* n, unordered_map<string, int>*
    record, int index) {
...
OSMGraph* OsmParser::LoadGraphFromFile(string filename, bool debug) {
  ...
  // sanity check, make sure the document loaded, and print something (anything)
  // from the document
#ifdef DEBUG
...
OSMGraph* OsmParser::read_nodes(pugi::xml_document* doc, bool debug) {
  ...
  for (way_node = parent_of_nodes.child("node"); way_node != nullptr;
      way_node = way_node.next_sibling("node")) {
    if (!way_node.attribute("id")) {
      std::cerr << "Improperly formed node missing id. Continuing."
        << std::endl;
      continue;
    }
    if (!way_node.attribute("lat")) {
      std::cerr << "Improperly formed node missing lat. ID: "
        << way_node.attribute("ref");
      std::cerr << ". Continuing" << std::endl;
      continue;
    }
    if (!way_node.attribute("lon")) {
      std::cerr << "Improperly formed node missing lon. ID: "
        << way_node.attribute("ref");
      std::cerr << ". Continuing" << std::endl;
      continue;
    }
    ...
    if (graph->Contains(id)) {
      std::cerr << "Attempted to add duplicate node. ID: "
        << way_node.attribute("ref");
      std::cerr << ". Continuing" << std::endl;
    }
...
float OsmParser::getLon(float lat, float lon,
    float centerLat, float centerLon) {
  float deltaLatitude = lat - centerLat;
  float deltaLongitude = lon - centerLon;
  float latitudeCircumference =
    40075160 * std::cos(OsmParser::asRadians(centerLat));
...
void OsmParser::read_adjacencies_to(OSMGraph* graph,
    pugi::xml_document* doc, bool debug) {
  unordered_map<string, set<string>> adjacencies =
    get_adjacency_list_from_file(doc, debug);

  for (auto it : adjacencies) {
    string from = it.first;
    set<string> others = it.second;
    if (!graph->Contains(from)) {
      std::cerr << "Node ID: " << from << " not found. Continuing."
        << std::endl;
      continue;
    }  // implicit else
    ...
    for (string to : others) {
      if (!graph->Contains(to)) {
        std::cerr << "Node ID: " << to << " not found. Continuing."
          << std::endl;
        continue;
      }  // implicit else
...
OsmParser::get_adjacency_list_from_file(pugi::xml_document* doc,
    bool debug) {
  // create a parent of all (interesting) nodes, then get the first node the the
  // "way" name
  pugi::xml_node parent_of_nodes = doc->first_child();
  ...
  // while the current way node is not equal to null, if it has a highway tag,
  // add it to the hashmap of ids to way nodes
  while (way_node != NULL) {
    for (pugi::xml_node tag_node = way_node.child("tag"); tag_node != nullptr;
  ...
  // get all the string ids of nodes that are a part of highway ways, we don't
  // care about the order and don't want repeats, so we use a unordered set
  std::unordered_set<std::string> highway_node_ids =
    std::unordered_set<std::string>();
  // for every highway way node, any child named "nd" is a node that is part of
  // the way, so add it to the highway node ids set
  for (auto & pair : highway_way_nodes) {
    pugi::xml_node way_node_child = pair.second.child("nd");
    while (way_node_child != NULL) {
      // if the node's name is not nd, we don't care about it, so skip.
  ...
  // we want an adjacency list that maps from a node's id to a list of nodes it
  // is connected to
  ...
  int num_edges = 0;
  // for every way node, adjacent "nd" nodes are connected, iterate through and
  // connect adjacent nodes in the adjacency list
  for (auto way_node_key_value : highway_way_nodes) {
    pugi::xml_node first = way_node_key_value.second.child("nd");
    pugi::xml_node second
      = way_node_key_value.second.child("nd").next_sibling();
    while (second != NULL) {
      ...
      if (strcmp(first.name(), "nd") == 0 && strcmp(second.name(), "nd") == 0) {
        adjacency_list[first.attribute("ref").value()]
          .insert(second.attribute("ref").value());
        adjacency_list[second.attribute("ref").value()]
          .insert(first.attribute("ref").value());
        num_edges++;
      }
      first = second;
